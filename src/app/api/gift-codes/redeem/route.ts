...
type GiftData = { name: string; type: string; description?: string; active: boolean; usedCount?: number; maxUses: number; startsAt: { toMillis?: () => number } | string; expiresAt: { toMillis?: () => number } | string };
...
export async function POST(req: Request) {
  try {
    ...
    await adminDb.runTransaction(async (transaction) => {
      ...
      transaction.set(requestRef, { userId: token.uid, userEmail: token.email || "", userFullName: userDoc.data()?.fullName || token.email?.split("@")[0] || "", rewardId: `gift_code_${giftDoc.id}`, rewardName: `${gift.name} (Gift code: ${code})`, pointsUsed: 0, type: gift.type, description: gift.description || "", status: "pending", createdAt: new Date(), updatedAt: new Date() });
    });
